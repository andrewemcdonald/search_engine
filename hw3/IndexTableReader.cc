...
DocIDTableReader *IndexTableReader::LookupWord(const std::string &word) {
  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    // Seek to correct postion in the document.
    ret = fseek(file_, next_offset, SEEK_SET);
    Verify333(ret == 0);

    // Read word_len field
    ret = fread(&header.word_len, sizeof(header.word_len), 1, file_);
    Verify333(ret == 1);

    // Read doctable_len field
    ret = fread(&header.doctable_len, sizeof(header.doctable_len), 1, file_);
    Verify333(ret == 1);
