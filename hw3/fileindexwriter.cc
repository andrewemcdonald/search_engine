...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  // MISSING:
  mires = WriteMemIndex(f, mi, dtres + sizeof(IndexFileHeader));
  filesize += mires;
...
static HWSize_t WriteDocidDocnameFn(FILE *f,
  ...
  // Write docName to file.
  for (int i = 0; i < slen_ho; i++) {
    res = fwrite(docName + i, sizeof(char), 1, f);
    if (res != 1) {
      // Write error.
      return 0;
    }
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  // Convert header to disk format.
  docid_element_header header = {docID_ho, num_pos_ho};
  header.toDiskFormat();

  // Find correct location to write header.
  ...
  // Write header
  res = fwrite(&header.docid, sizeof(header.docid), 1, f);
  if (res != 1) {
    // Write error
    return 0;
  }

  res = fwrite(&header.num_positions, sizeof(header.num_positions), 1, f);
  if (res != 1) {
    // Write error
    return 0;
  }
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    res = fwrite(&position.position, sizeof(position.position), 1, f);
    if (res != 1) {
      // Write error
      return res;
    }
  ...
  return sizeof(header) + num_pos_ho * sizeof(position);
}
...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  // Go to right place to write header.
  res = fseek(f, offset, SEEK_SET);
  if (res != 0) {
    // Seek error
    return 0;
  ...
  // Write header
  res = fwrite(&header.word_len, sizeof(header.word_len), 1, f);
  if (res != 1) {
    // Write error
    return 0;
  }

  res = fwrite(&header.doctable_len, sizeof(header.doctable_len), 1, f);
  if (res != 1) {
    // Write error
    return 0;
  }
  ...
  for (int i = 0; i < wordlen_ho; i++) {
    res = fwrite(wds->word + i, sizeof(char), 1, f);
    if (res != 1) {
      // Write error
      return 0;
    }
...
static HWSize_t WriteHeader(FILE *f,
  ...
  // Start at beggining of doctable
  res = fseek(f, sizeof(header), SEEK_SET);
  if (res != 0) {
    // Seek error
    return 0;
  }
  ...
  for (HWSize_t i = 0; i < cslen; i++) {
    res = fread(&my_byte, sizeof(my_byte), 1, f);
    if (res != 1) {
      // Read error
      return 0;
    }
    crcobj.FoldByteIntoCRC(my_byte);
  }
...
static HWSize_t WriteBucketRecord(FILE *f,
  ...
  // Write br.chain_len
  res = fwrite(&br.chain_len, sizeof(br.chain_len), 1, f);
  if (res != 1) {
    // Write error
    return 0;
  }

  // Write br.bucket_position
  res = fwrite(&br.bucket_position, sizeof(br.bucket_position), 1, f);
  if (res != 1) {
    // Write error
    return 0;
  }
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // Convert element position to architecture-neutral format.
      element_position_rec pos = {nextelpos};
      pos.toDiskFormat();

      // Go to correct spot to write element position.
      res = fseek(f, (sizeof(element_position_rec) * j) + offset, SEEK_SET);
      if (res != 0) {
        // Seek error
        return 0;
      }
      ...
      res = fwrite(&pos.element_position, sizeof(element_position_rec), 1, f);
      if (res != 1) {
        // Write error
        return 0;
      }
      ...
      kv = NULL; // To avoid compiler warning
      LLIteratorGetPayload(it, (LLPayload_t*)&kv);

      // Go to correct spot to write element.
      res = fseek(f, nextelpos, SEEK_SET);
      if (res != 0) {
        // Seek error
        return 0;
      }
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    ...
    // Write the current bucket offset
    ...
    // Update next_bucket_rec_offset
    next_bucket_rec_offset += res;

    if (NumElementsInLinkedList(ht->buckets[i]) != 0) {
      // Bucket has elements, write the current bucket
