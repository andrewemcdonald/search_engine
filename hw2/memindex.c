...
#define M_INDEX_BUCKET_INIT 128
...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    // Malloc space for new WordDocSet
    ...
    // Allocate and insert new hashtable
    wds->docIDs = AllocateHashTable(M_INDEX_BUCKET_INIT);
    Verify333(wds->docIDs != NULL);

    InsertHashTable(index, kv, &hitkv);
  ...
  InsertHashTable(wds->docIDs, kv, &hitkv);
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  while (!HTIteratorPastEnd(iter)) {
    // Add search result to retlist
    SearchResultPtr sr = (SearchResultPtr)malloc(sizeof(SearchResult));
    Verify333(sr != NULL);

    HTIteratorGet(iter, &kv);
    sr->docid = kv.key;
    sr->rank = NumElementsInLinkedList((LinkedList)kv.value);
    // Should have checked for failure here.
    PushLinkedList(retlist, (LLPayload_t)sr);
