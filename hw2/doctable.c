...
// A hashtable free function that frees the given payload.
void freePayloadFunc(HTValue_t t) { free(t); }
...
void FreeDocTable(DocTable table) {
  ...
  // Must free the docname pointed to by payload.
  FreeHashTable(table->docid_to_docname, &freePayloadFunc);

  // Must free docid pointed to by payload.
  FreeHashTable(table->docname_to_docid, &freePayloadFunc);
...
DocID_t DTRegisterDocumentName(DocTable table, char *docname) {
  ...
  // Should have done DTLookupDocumentName instead.
  key = FNVHash64((unsigned char *)doccopy, strlen(doccopy));
  res = LookupHashTable(table->docname_to_docid, key, &oldkv);
  ...
  *docid = table->max_id;
  ...
  kv.key = key;
  kv.value = docid;
  // Note that use of NULL here is acceptable because we know that no
  // value should be returned.
  InsertHashTable(table->docname_to_docid, kv, NULL);

  // STEP 4.
  // Set up the key/value for the docname_to_docid mapping, and
  // do the insert.

  kv.key = (HTKey_t) table->max_id;
  kv.value = doccopy;
  // Note that use of NULL here is acceptable because we know that
  // no value should be returned.
  InsertHashTable(table->docid_to_docname, kv, NULL);
